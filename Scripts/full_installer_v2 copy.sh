...
## ============================================================================
# Vista previa de paquetes obligatorios por categoría
show_required_packages_preview() {
    local categories=("$@")
    echo
    info "🔍 Paquetes obligatorios por categoría:"
    for cat in "${categories[@]}"; do
        ...
        if [[ ${#pkgs[@]} -gt 0 ]]; then
            echo
            # Mostrar categoría con descripción
            local desc
            desc=$(yq -r ".categories[] | select(.id==\"${cat}\") | .description // \"Sin descripción\"" "$PACKAGES_YAML")
            info "📁 Categoría: $cat"
            echo "   📋 $desc"
...
## ============================================================================
# Instalación de paquetes obligatorios (optional=false)
install_required_categories() {
    local install_mode="$1"; shift
    local categories=("$@")
    info "🚀 Iniciando instalación de paquetes obligatorios: ${categories[*]}"
    echo
    for cat in "${categories[@]}"; do
        ...
        # Saltar categorías sin paquetes obligatorios
        if [[ 
            ${#pkgs[@]} -eq 0 ]]; then
            continue
        fi
        ...
        info "🎯 Categoría: $cat"
        echo "   📋 $desc"
        echo "   📊 Paquetes obligatorios: ${#pkgs[@]}"
        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        for pkg in "${pkgs[@]}"; do
            echo "📦 $pkg"
            install_package_simple "$pkg" "$install_mode" || true
    ...
    if [[ ${#categories[@]} -eq 0 ]]; then
        ...
        case "$install_mode" in
            ...
            "selective")
                ...
                ;;
            "required_only")
                # Contar paquetes requeridos y opcionales usando yq
                local required_count
                required_count=$(yq -r '[.categories[].packages[] | select(.optional == false or .optional == null)] | length' "$PACKAGES_YAML")
                local optional_count
                optional_count=$(yq -r '[.categories[].packages[] | select(.optional == true)] | length' "$PACKAGES_YAML")
                ...
                # Mostrar vista previa de los paquetes obligatorios
                show_required_packages_preview "${categories[@]}"
                if ask_yes_no "¿Continuar con la instalación de paquetes obligatorios?"; then
                    install_required_categories "$install_mode" "${categories[@]}"
                else
                    info "Instalación cancelada"
                fi
                ;;
